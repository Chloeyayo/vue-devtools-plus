...
const VUE_INTERNAL_COMPONENT_NAMES = {
  Transition: true,
...
const ELEMENT_UI_INTERNAL_COMPONENT_NAMES = {
  Bar: true,
  LabelWrap: true,
  Next: true,
  Prev: true,
...
const ELEMENT_UI_PACKAGE_RE = /(?:^|\/)packages\/(?:alert|aside|autocomplete|avatar|badge|breadcrumb|button|calendar|card|carousel|cascader|checkbox|collapse|color-picker|container|date-picker|dialog|divider|dropdown|form|header|icon|image|input|input-number|link|main|menu|message|message-box|notification|pagination|popover|progress|radio|rate|scrollbar|select|slider|steps|switch|table|tabs|tag|time-picker|time-select|timeline|tooltip|transfer|tree|upload)(?:\/|$)/
...
function normalizeComponentName (name) {
  return classify(String(name || '').replace(/[<>]/g, '').replace(/\s+/g, '-'))
}

function isDependencyComponentFile (file) {
  const normalized = normalizeComponentFile(file)
...
function hasElementUIAncestor (component) {
  let parent = component.parent
  while (parent) {
    const parentName = normalizeComponentName(parent.name || parent.displayName || parent._componentTag)
    const parentFile = parent.file || parent.__file
    if (isElementUIComponent(parentName, parentFile)) {
      return true
    }
...
export function isBusinessComponent (component = {}) {
  const file = component.file || component.__file
  const name = normalizeComponentName(component.name || component.displayName || component._componentTag)
  if (!name && !file) {
    return true
  }

  if (VUE_INTERNAL_COMPONENT_NAMES[name]) {
    return false
  }

  if (isElementUIComponent(name, file)) {
    return false
  }

  if (ELEMENT_UI_INTERNAL_COMPONENT_NAMES[name] && hasElementUIAncestor(component)) {
    return false
  }
