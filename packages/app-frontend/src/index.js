...
const SHORTCUT_EVENT_RETRIES = 20
const SHORTCUT_EVENT_RETRY_DELAY = 50
const DUPLICATE_SHORTCUT_WINDOW = 250
...
function initApp (shell) {
  shell.connect(bridge => {
    ...
    initSharedData({
      ...
      bridge.on('inspect-state-path', entry => {
        ensurePaneShown(() => {
          inspectStatePath(entry, store)
        })
      })

      bridge.on('devtools-shortcut', payload => {
        ensurePaneShown(() => {
          handleDevtoolsShortcut(payload)
        })
      })
...
function handleDevtoolsShortcut (payload) {
  if (!payload || !payload.name) return
  if (isDuplicateDevtoolsShortcut(payload)) return

  if (payload.name === 'toggle-component-selector') {
    openRouteAndDispatchShortcut('components', COMPONENT_SELECTOR_SHORTCUT_EVENT)
  } else if (payload.name === 'enhanced-search') {
    openRouteAndDispatchShortcut('enhanced-search', ENHANCED_SEARCH_SHORTCUT_EVENT)
  }
}

function isDuplicateDevtoolsShortcut (payload) {
  if (!payload.eventId) return false

  const now = Date.now()
  recentDevtoolsShortcuts = recentDevtoolsShortcuts.filter(shortcut => {
    return now - shortcut.time < DUPLICATE_SHORTCUT_WINDOW
  })

  if (recentDevtoolsShortcuts.some(shortcut => shortcut.id === payload.eventId)) {
    return true
  }

  recentDevtoolsShortcuts.push({
    id: payload.eventId,
    time: now
  })
  return false
...
function openRouteAndDispatchShortcut (routeName, eventName) {
  const dispatch = () => dispatchRetriedShortcutEvent(eventName)

  if (router.currentRoute.name === routeName) {
    Vue.nextTick(dispatch)
  } else {
...
function inspectStatePath (entry, store) {
  if (!entry || entry.uid == null || !entry.path) return

  store.dispatch('components/selectInstance', entry.uid)

  const dispatch = () => dispatchRetriedShortcutEvent(INSPECT_STATE_PATH_EVENT, {
    entry
  })

  if (router.currentRoute.name === 'components') {
    Vue.nextTick(dispatch)
  } else {
    router.push({ name: 'components' }, () => {
      Vue.nextTick(dispatch)
...
function dispatchRetriedShortcutEvent (eventName, extraDetail = null) {
  const detail = {
    handled: false,
    ...(extraDetail || {})
  }
  ...
  const dispatch = () => {
    ...
    if (!detail.handled && tries < SHORTCUT_EVENT_RETRIES) {
      window.setTimeout(dispatch, SHORTCUT_EVENT_RETRY_DELAY)
    }
  }
